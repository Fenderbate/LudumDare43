...
func start_demon():
	...
	$Tween.start()
	$Tween.interpolate_property($Dark,"self_modulate",$Dark.self_modulate,Color(1,1,1,0.8),20,Tween.TRANS_CUBIC,Tween.EASE_OUT)
	$Tween.start()
	...
	match object.name:
		"Demon":
			if !up:
				...
				$Tween.start()
				$Tween.interpolate_property($Dark,"self_modulate",$Dark.self_modulate,Color(1,1,1,0),0.5,Tween.TRANS_CUBIC,Tween.EASE_OUT)
				$Tween.start()
